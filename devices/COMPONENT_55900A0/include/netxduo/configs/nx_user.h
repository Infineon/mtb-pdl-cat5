...
/*
 *  Map NX_RAND to something that generates better random numbers than the default rand() function.
 */
#define NX_RAND() thread_ap_rbg_rand(1)
...
*/
/* Set the ARP expiration rate to 5 minutes to enable ARP offload host auto reply feature  */
...
*/
/* The amount of header space required for WHD is 4 bytes making the full amount 16 bytes.
 * Ethernet header (14 bytes) + WHD (4) + padding to 4 byte boundary (2) */
#define NX_PHYSICAL_HEADER          (14 + 4 + 2)
