...
#define TX_THREAD_SYSTEM_RETURN_CHECK(c)    \
    (c) = (ULONG) _tx_thread_preempt_disable; (c) = (c) | TX_THREAD_GET_SYSTEM_STATE();
#endif
...
#endif // ifndef TX_LOWEST_SET_BIT_CALCULATE
...
#endif // ifndef TX_THREAD_STACK_CHECK


/* Define default post thread delete macro to whitespace, if it hasn't been defined previously
   (typically in tx_port.h).  */
...
/* Define default post thread reset macro to whitespace, if it hasn't been defined previously
   (typically in tx_port.h).  */
...
/* Define the thread create internal extension macro to whitespace, if it hasn't been defined
   previously (typically in tx_port.h).  */
...
#endif
#endif // ifndef TX_DISABLE_PREEMPTION_THRESHOLD
...
THREAD_DECLARE  VOID            (* _tx_thread_application_stack_error_handler)(
