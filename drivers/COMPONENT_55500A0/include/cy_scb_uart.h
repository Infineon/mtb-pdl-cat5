...
typedef struct stc_scb_uart_config
{
    ...
    */
    BOOL8        enableMutliProcessorMode;
    ...
    */
    BOOL8        irdaEnableLowPowerReceiver;
...
* \{
*/
#define CY_SCB_UART_IRDA_LP_OVS16      (1u)   /**< IrDA in low-power mode oversampled by 16   */
#define CY_SCB_UART_IRDA_LP_OVS32      (2u)   /**< IrDA in low-power mode oversampled by 32   */
#define CY_SCB_UART_IRDA_LP_OVS48      (3u)   /**< IrDA in low-power mode oversampled by 48   */
#define CY_SCB_UART_IRDA_LP_OVS96      (4u)   /**< IrDA in low-power mode oversampled by 96   */
#define CY_SCB_UART_IRDA_LP_OVS192     (5u)   /**< IrDA in low-power mode oversampled by 192  */
#define CY_SCB_UART_IRDA_LP_OVS768     (6u)   /**< IrDA in low-power mode oversampled by 768  */
#define CY_SCB_UART_IRDA_LP_OVS1536    (7u)   /**< IrDA in low-power mode oversampled by 1536 */
...
#define CY_SCB_UART_RECEIVE_ACTIVE         (0x01u)
...
#define CY_SCB_UART_TRANSMIT_ACTIVE    (0x01u)

/**
* All data elements specified by \ref Cy_SCB_UART_Transmit have been loaded
* into the TX FIFO
*/
#define CY_SCB_UART_TRANSMIT_IN_FIFO   (0x02u)
...
*/
#define CY_SCB_UART_RB_FULL_EVENT          (0x08u)
...
*/
#define CY_SCB_UART_RECEIVE_ERR_EVENT      (0x10u)
...
#define CY_SCB_UART_INIT_KEY       (0x00ABCDEFu)
...
#define CY_SCB_UART_IS_ADDRESS_VALID(addr)          ((addr) <= 0xFFu)
#define CY_SCB_UART_IS_ADDRESS_MASK_VALID(mask)     ((mask) <= 0xFFu)
#define CY_SCB_UART_IS_DATA_WIDTH_VALID(width)      ( ((width) >= 5u) && ((width) <= 9u) )
#define CY_SCB_UART_IS_OVERSAMPLE_VALID(ovs, mode, lpRx)    ( ((CY_SCB_UART_STANDARD  == (mode)) || (CY_SCB_UART_SMARTCARD == (mode))) ? \
                                                              (((ovs) >= 8u) && ((ovs) <= 16u)) :                                      \
                                                              (BOOL8) ((lpRx) ? CY_SCB_UART_IS_IRDA_LP_OVS_VALID(ovs) : TRUE) )

#define CY_SCB_UART_IS_RX_BREAK_WIDTH_VALID(base, width)    ( ((width) >= (_FLD2VAL(SCB_RX_CTRL_DATA_WIDTH, (base)->RX_CTRL) + 3u)) && \
                                                              ((width) <= 16u) )
#define CY_SCB_UART_IS_TX_BREAK_WIDTH_VALID(width)          ( ((width) >= 4u) && ((width) <= 16u) )

#define CY_SCB_UART_IS_MUTLI_PROC_VALID(mp, mode, width, parity)    ( (mp) ? ((CY_SCB_UART_STANDARD  == (mode)) && ((width) == 9u) && \
...
__STATIC_INLINE uint32_t Cy_SCB_UART_GetOverSample(CySCB_Type const *base)
{
    return (_FLD2VAL(SCB_CTRL_OVS, SCB_CTRL(base))+1u);
}
...
__STATIC_INLINE uint32_t Cy_SCB_UART_GetStopBits(CySCB_Type const *base)
{
    return (_FLD2VAL(SCB_UART_TX_CTRL_STOP_BITS, SCB_UART_TX_CTRL(base))+1u);
}
...
__STATIC_INLINE void Cy_SCB_UART_PutString(CySCB_Type *base, char_t const string[])
{
    CY_ASSERT_L1(CY_SCB_IS_BUFFER_VALID(string, 1u));
