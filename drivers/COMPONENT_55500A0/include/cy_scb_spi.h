...
__STATIC_INLINE BOOL8     Cy_SCB_SPI_IsTxComplete  (CySCB_Type const *base);
...
*/
#define CY_SCB_SPI_TRANSFER_IN_FIFO    (0x02u)
...
#define CY_SCB_SPI_DEFAULT_TX  (0x0000FFFFu)
...
#define CY_SCB_SPI_INIT_KEY        (0x00ABCDEFu)
...
#define CY_SCB_SPI_IS_OVERSAMPLE_VALID(ovs, mode)   ( (CY_SCB_SPI_MASTER == (mode)) ? (((ovs) >= 2u) && ((ovs) <= 16u)) : TRUE )
#define CY_SCB_SPI_IS_DATA_WIDTH_VALID(width)       ( ((width) >= 4u) && ((width) <= 16u) )
#define CY_SCB_SPI_IS_SS_POLARITY_VALID(polarity)   ( (0u == ((polarity) & (~0x0Fu))) )
#define CY_SCB_SPI_IS_BUFFER_VALID(txBuffer, rxBuffer, size)  ( ((size) > 0u)  && \
...
__STATIC_INLINE void Cy_SCB_SPI_SetActiveSlaveSelectPolarity(CySCB_Type *base,
                                ...
                                cy_en_scb_spi_polarity_t polarity)
{
    uint32_t mask = _VAL2FLD(CY_SCB_SPI_CTRL_SSEL_POLARITY, (0x01u << ((uint32_t)slaveSelect)));
